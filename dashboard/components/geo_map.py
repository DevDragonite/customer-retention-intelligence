...
def render_churn_map(predictions_df: pd.DataFrame, churn_por_estado: Dict) -> None:
    ...
    fig.add_trace(go.Scattergeo(
        ...
        marker=dict(
            ...
            colorbar=dict(
                title=dict(text="Churn %", font=dict(color="#F8F2DC")),
    ...
    fig.update_geos(
        scope="south america",
        showland=True, landcolor="rgb(28, 20, 16)",
        showocean=True, oceancolor="rgb(20, 14, 10)",
    ...
    fig.update_layout(height=600, showlegend=False, margin=dict(l=0, r=0, t=50, b=0))
...
def render_delivery_vs_churn(features_df: pd.DataFrame) -> None:
    ...
    fig.add_trace(go.Scatter(
        x=estado_stats["delay_pct"],
        y=estado_stats["churn_pct"],
        mode="markers+text",
    ...
    if len(estado_stats) > 2:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines",
            line=dict(color="#DEA47E", dash="dash", width=2),
            name="Trend",
            showlegend=True,
        ))
